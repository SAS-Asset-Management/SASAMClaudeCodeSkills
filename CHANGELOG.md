...
### Fixed
- **ensemble MCP: ensemble_create_project_task** sent `title` but the Ensemble API requires `name`, so every call 422'd and no task was ever created. Now sends `name`, and gains optional `workstream_id` and `phase_no` so tasks land inside the plan tree instead of arriving unassigned. Empty/None fields are omitted from the request body.
- **ensemble MCP: ensemble_update_project_task** sent `{"status": ""}` when called without a status, blanking the task's status on the backend (partial-update PATCH applies explicitly-sent empty strings). Empty fields are now omitted; the tool also accepts optional `name`/`description`, and refuses an update with no fields set. Plugin bumped 1.3.0 → 1.3.1; corrected `marketplace.json` version drift for the ensemble entry (1.2.0 → 1.3.1).
