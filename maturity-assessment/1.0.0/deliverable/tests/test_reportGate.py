...
def test_completeLedgerOpensGate():
    verdict = reportGate.evaluateGate(makeLedger(), makePack())
    assert verdict["open"] is True
    assert verdict["reasons"] == []


def test_missingSubjectClosesGate():
    ledger = makeLedger()
    del ledger["subjects"]["03_gamma"]
    verdict = reportGate.evaluateGate(ledger, makePack())
...
def test_nullFinalScoreClosesGate():
    ledger = makeLedger()
...
def test_halfSetSayScoreClosesGate():
    ledger = makeLedger()
...
def test_halfSetDoScoreClosesGate():
    ledger = makeLedger()
...
def test_openDisputeClosesGate():
    ledger = makeLedger()
...
def test_resolvedDisputeDoesNotCloseGate():
    ledger = makeLedger()
    ledger["subjects"]["03_gamma"]["disputes"] = [
        {
            "id": "DSP-08",
