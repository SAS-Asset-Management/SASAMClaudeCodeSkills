...
from sasdockit import doctor
from sasdockit.grid.model import parse_grid
from sasdockit.ir.model import parse_idoc
from sasdockit.profile import comprehension as comprehension_mod
from sasdockit.profile import schema, store
from sasdockit.qa.gate import run_qa
...
def _discover_learn_reports(profile: dict, shell_path) -> list[dict]:
    ...
    """
    from sasdockit.qa import report as vreport
...
def _extract_blend(args) -> int:
    ...
    from sasdockit.profile import blend as blend_mod
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            if loaded.kind == "docx":
                from sasdockit.formats.docx import extract as fmt_extract
            elif loaded.kind == "pptx":
                from sasdockit.formats.pptx import extract as fmt_extract
            elif loaded.kind == "xlsx":
                from sasdockit.formats.xlsx import extract as fmt_extract
            else:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="sasdocx")
    ...
    if args.cmd == "extract":
        ...
        try:
            if suffix == ".docx":
                from sasdockit.formats.docx import extract as docx_extract

                profile_json = docx_extract.extract(path, args.name, scope=args.scope)
            elif suffix == ".pptx":
                from sasdockit.formats.pptx import extract as pptx_extract

                profile_json = pptx_extract.extract(path, args.name, scope=args.scope)
            elif suffix == ".xlsx":
                from sasdockit.formats.xlsx import extract as xlsx_extract
    ...
    if args.cmd == "generate":
        ...
        try:
            if loaded.kind == "docx":
                from sasdockit.formats.docx import generate as docx_generate
                ...
                from sasdockit.formats.pptx import generate as pptx_generate
                ...
                from sasdockit.formats.xlsx import generate as xlsx_generate
            ...
            return 1
        from sasdockit.qa import report as vreport
        from sasdockit.qa import visual as vqa
    ...
    if args.cmd == "refine":
        ...
        return 0
    if args.cmd == "learn":
        from sasdockit.profile import overrides as overrides_mod
        from sasdockit.qa import report as vreport
    ...
    if args.cmd == "compare-profiles":
        from sasdockit.profile import compare
