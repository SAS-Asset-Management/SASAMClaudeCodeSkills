...
re-derives raw lxml name glue per format (the same intent :mod:`sasdockit.ooxml.pack`
...
- :func:`qname` / :func:`make_qn` - the namespace-SPECIFIC direction. A walker
  ...
  color helpers already used, and is distinct from :func:`sasdockit.ooxml.pack.qn`,
...
Namespace URIs are sourced from :data:`sasdockit.ooxml.pack.NAMESPACES`, so there
...
def make_qn(prefix: str) -> Callable[[str], str]:
    ...
    :data:`sasdockit.ooxml.pack.NAMESPACES`::
    ...
    # :mod:`sasdockit.ooxml.pack`); :func:`local_name` / :func:`qname` stay usable
    # from import-safe (no-lxml) callers such as :mod:`sasdockit.common.color`.
    from sasdockit.ooxml.pack import NAMESPACES
