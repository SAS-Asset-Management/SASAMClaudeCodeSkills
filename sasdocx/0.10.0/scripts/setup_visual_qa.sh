...
#
# One-command setup for the SASdocX visual QA gate's SYSTEM dependencies
...
echo "SASdocX: installing visual QA system dependencies (LibreOffice, Poppler, Tesseract)"
...
Install these manually, then re-run `python scripts/sasdockit/cli.py doctor`:
...
echo "  python scripts/sasdockit/cli.py doctor"
