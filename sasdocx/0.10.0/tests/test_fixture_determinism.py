...
# Raster image media. Pillow-rendered PNGs (the _saslib wordmark/curve) are NOT
...
def _load_example_builder(stem: str) -> ModuleType:
    """Import an ``examples/builders/build_sasdocx_<stem>.py`` builder by path.

    The example builders do ``from _saslib import ...``, so their directory must
    ...
    path = _EX_BUILDERS / f"build_sasdocx_{stem}.py"
...
class ExampleTemplateDeterminismGuard(unittest.TestCase):
    ...
    def test_docx_example_matches_committed(self) -> None:
        self._check("docx", "sasdocx_template.docx")

    def test_pptx_example_matches_committed(self) -> None:
        self._check("pptx", "sasdocx_template.pptx")

    def test_xlsx_example_matches_committed(self) -> None:
        self._check("xlsx", "sasdocx_template.xlsx")
