...
Produces ``examples/templates/sasdocx_template.docx``: a 100% synthetic
(``SASdocX Corp``, never proprietary) Word template that stresses the sasdocx
...
Components authored (each is a real OOXML structure, not a text approximation):
  ...
  NUMBERING (real ``word/numbering.xml``, referenced via ``w:numPr`` from named
    ...
    * a 2-level BULLET list  -> styles "SASdocX Bullet L1" / "SASdocX Bullet L2";
    * a 1-level NUMBERED list -> style "SASdocX Number L1".

  TABLES: a custom ``w:type="table"`` style "SASdocX Table" (header-row
  ...
  FIGURE: two real inline PNG figures, each with a ``SEQ Figure`` caption -
    Figure 1 is the shared text-only SASdocX wordmark (image1.png) and Figure 2
    is a real rising growth curve (image2.png, a distinct media part).

  CALLOUT: a paragraph style "SASdocX Callout" with shading + a box border.
  QUOTE: a paragraph style "SASdocX Quote" with a navy/amber editorial treatment.

  HEADER / FOOTER: the shared SASdocX wordmark in the default header, and a
...
Run:
    PYTHONPATH=scripts .venv/bin/python examples/builders/build_sasdocx_docx.py
"""
...
OUT = Path(__file__).resolve().parents[1] / "templates" / "sasdocx_template.docx"
...
# Synthetic SASdocX Corp brand palette (made-up; never proprietary).
BRAND_NAVY = "16213F"
BRAND_TEAL = "2B7CD3"
BRAND_AMBER = "E0742B"
BRAND_LIGHT = "EAF1FF"
...
def _add_styled_run(paragraph, text, rstyle, *, color=None, font=None):
    ...
    Used to put a ``SASdocXEmphasis`` / ``SASdocXLeadIn`` mark on a body run so
...
# place: a:fontScheme (Arial major / Calibri minor) + a:clrScheme (SASdocX
...
def _rewrite_theme_clrscheme(scheme) -> None:
    """Rewrite the ``a:clrScheme`` children to the SASdocX palette (DOCX-A2).
    ...
    SASdocX navy/light directly. Slots not named by the brand map keep a sensible
    branded value so every clrScheme child resolves to a real SASdocX-family hex.
    """
...
def _add_table_style(styles):
    ...
    st = _sub(styles, "style", type="table", styleId="SASdocXTable")
    st.set(_w("customStyle"), "1")
    _sub(st, "name", val="SASdocX Table")
...
def _add_section_number_style(styles):
    """A heading-number variant paragraph style (DOCX-A5): SASdocXSectionNumber.
    ...
    _add_paragraph_style(
        styles,
        "SASdocXSectionNumber",
        "SASdocX Section Number",
        color=BRAND_NAVY,
        bold=True,
        size_pt=11,
    )
...
def _add_character_styles(styles):
    ...
    if "SASdocXEmphasis" not in have:
...
def _add_lead_style(styles):
    """A custom 'SASdocX Lead' intro-paragraph style (DOCX-A5): teal 13pt lead.
...
def _add_table_compact_style(styles):
    """A SECOND custom ``w:type='table'`` style (DOCX-A7): SASdocXTableCompact.
    ...
    bold). Authored ALONGSIDE SASdocXTable (which stays ``table.default``); the
    resolver binds the first custom table style, so adding this second one after it
    is order-stable.
    """
    st = _sub(styles, "style", type="table", styleId="SASdocXTableCompact")
    st.set(_w("customStyle"), "1")
    _sub(st, "name", val="SASdocX Table Compact")
    _sub(st, "basedOn", val="TableNormal")
    _sub(st, "uiPriority", val="99")
    tblPr = _sub(st, "tblPr")
    borders = _sub(tblPr, "tblBorders")
...
def _build_styles(doc):
    ...
    _add_paragraph_style(
        styles,
        "SASdocXCoverSubtitle",
        "SASdocX Cover Subtitle",
        color=BRAND_TEAL,
        size_pt=14,
    )
    _add_paragraph_style(
    ...
    _add_list_style(
        styles, "SASdocXBulletL1", "SASdocX Bullet L1", num_id=1, ilvl=0
    )
    _add_list_style(
        styles, "SASdocXBulletL2", "SASdocX Bullet L2", num_id=2, ilvl=1
    )
    _add_list_style(
        styles, "SASdocXNumberL1", "SASdocX Number L1", num_id=3, ilvl=0
    )
...
def _populate_numbering(root) -> None:
    """Fill an existing ``w:numbering`` element with SASdocX abstractNum/num defs.
...
def _build_footnotes_xml() -> bytes:
    ...
    txt = _sub(p, "r")
    t = _sub(txt, "t")
    t.text = " SASdocX Corp is a fictional company used only for testing."
...
def _cover_title_sdt():
    ...
    _sub(sdtPr, "tag", val="sasdocx_title")
    ...
    p = _sub(sdtContent, "p")
    pPr = _sub(p, "pPr")
    _sub(pPr, "pStyle", val="SASdocXCoverTitle")
    r = _sub(p, "r")
    rpr2 = _sub(r, "rPr")
    ...
    _sub(rpr2, "b")
    t = _sub(r, "t")
    t.text = "SASdocX Brand Operations Review"
    return sdt
...
def _cover_background_shape():
    ...
    docpr.set("id", "400")
    docpr.set("name", "SASdocXCoverBackdrop")
...
def _build_cover(doc):
    ...
    sub_p = _p(
        doc,
        "Annual Brand Operations Review - SASdocX Corp (synthetic)",
        "SASdocXCoverSubtitle",
    )
...
def _build_cover_kpi_summary(doc):
    ...
    Exercises the SECOND custom table style ``SASdocXTableCompact`` and its
    ...
    table.style = "SASdocX Table Compact"
    tblPr = table._tbl.tblPr
    _sub(
        tblPr,
        "tblLook",
...
def _build_index_front_matter(doc):
    ...
    def _tot_entries(d):
        ...
        _toc_entry(
            d, "Table 1. SASdocX FY2026 quarterly revenue", 5, style="TableofFigures"
        )
        _toc_entry(
            d, "Table 2. SASdocX risk and readiness matrix", 6, style="TableofFigures"
        )
        _toc_entry(
            d, "Table 3. SASdocX program rollout matrix", 7, style="TableofFigures"
        )
    ...
    def _tof_entries(d):
        _toc_entry(d, "Figure 1. SASdocX Corp wordmark", 2, style="TableofFigures")
        _toc_entry(d, "Figure 2. SASdocX growth curve", 6, style="TableofFigures")
...
def _build_table(doc):
    ...
    _pr(doc, "SASdocX quarterly revenue", "Heading2", font="Arial", color=BRAND_NAVY)
    ...
    _seq_caption(
        ...
        "Table",
        "Table",
        "SASdocX FY2026 quarterly revenue by region (synthetic data).",
    )


def _build_figure(doc, logo_rid, logo_cx, logo_cy):
    _p(doc, "SASdocX wordmark", "Heading2")
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run()
    run._r.append(
        _inline_drawing(logo_rid, logo_cx, logo_cy, "SASdocXLogoFigure", 100)
    )
    _seq_caption(doc, "Figure", "Figure", "SASdocX Corp wordmark (synthetic).")
...
def _build_callout(doc):
    ...
    # SASdocXLeadIn small-caps navy lead-in opens it.
    p = _p(doc, "", "SASdocXCallout")
    _add_styled_run(p, "Note. ", "SASdocXLeadIn", color=BRAND_NAVY)
    run = p.add_run(
        "SASdocX Corp is a synthetic, fictional company used to demonstrate an "
...
def _build_editorial_quote(doc):
    ...
    # SASdocXEmphasis teal-bold mark highlights the operative phrase.
    p = _p(doc, "", "SASdocXQuote")
    from docx.shared import RGBColor

    run = p.add_run("Design rule: preserve proven structure only while it ")
    run.font.color.rgb = RGBColor.from_string(BRAND_NAVY)
    _add_styled_run(
        p, "improves the final artifact", "SASdocXEmphasis", color=BRAND_TEAL
    )
    run2 = p.add_run(
...
def _build_risk_matrix(doc):
    ...
    _seq_caption(
        doc, "Table", "Table", "SASdocX risk and readiness matrix (synthetic)."
    )


def _build_footnote_paragraph(doc, footnote_id):
    _pr(doc, "SASdocX footnote demo", "Heading2", font="Arial", color=BRAND_NAVY)
    body = doc.add_paragraph("SASdocX Corp")
...
def _build_demo_body(doc):
    ...
    _pr(
        doc,
        "A complete, on-brand SASdocX Corp template, captured and ready to drive "
        "branded generation.",
        "SASdocXLead",
        font="Calibri",
        color=BRAND_TEAL,
    ...
    _pr(
        doc,
        "This brief summarizes how the SASdocX Corp brand system is applied "
    ...
    _pr(
        ...
        "SASdocX brand office. Revenue, growth, and regional splits are "
...
def _build_header_footer(doc, logo_rid, cx, cy):
    ...
    run._r.append(_inline_drawing(logo_rid, cx, cy, "SASdocXHeaderLogo", 200))
...
def _add_two_column_section(doc):
    ...
    after the portrait body, carrying a short 'SASdocX in brief' two-column
    ...
    _p(doc, "SASdocX in brief", "Heading2")
    _pr(
        doc,
        "SASdocX Corp is a synthetic brand used to exercise the on-brand "
...
def _add_landscape_section(doc, curve_rid=None, fig_cx=0, fig_cy=0):
    ...
    for child in list(cols):
        cols.remove(child)
    cols.set(_w("num"), "1")
    _p(doc, "SASdocX landscape appendix", "Heading1")
    doc.add_paragraph(
    ...
    _seq_caption(
        doc, "Table", "Table", "SASdocX FY2026 program rollout matrix (synthetic)."
    )
    ...
    if curve_rid is not None:
        _p(doc, "SASdocX growth curve", "Heading2")
        fp = doc.add_paragraph()
        fp.alignment = WD_ALIGN_PARAGRAPH.CENTER
        frun = fp.add_run()
        frun._r.append(
            _inline_drawing(curve_rid, fig_cx, fig_cy, "SASdocXGrowthFigure", 300)
        )
        _seq_caption(
            doc,
            "Figure",
            "Figure",
            "SASdocX Corp growth curve (synthetic illustration).",
        )
    return new_section
...
def build(out: Path = OUT) -> Path:
    ...
    # the fontScheme (Arial major / Calibri minor) and clrScheme (SASdocX
    ...
    # 4) Shared logo image: a generated text-only SASdocX wordmark. The SAME
    # wordmark every SASdocX template embeds (CR-LOGO-MARK). Related to the
    ...
    # (CR-FIG2-CURVE).
    logo_blob = sasdocx_mark_png(640, 160)  # wide wordmark (image1.png)
    curve_blob = sasdocx_curve_png(480, 200)  # ~12:5 growth curve (image2.png)
