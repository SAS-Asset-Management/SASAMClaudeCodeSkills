...
Produces ``examples/templates/sasdocx_template.pptx``: a 100% synthetic
(made-up "SASdocX Corp" brand, never proprietary) deck that stresses the
saspptx extractor/generator across as many PowerPoint component types as
python-pptx (plus a little raw lxml) can author from the default template:
  ...
  * A PICTURE: the shared, deterministic text-only SASdocX wordmark PNG from
    ``_saslib.sasdocx_mark_png`` - no external / proprietary asset on disk -
  ...
  * A logo-like GROUPED-SHAPE mark + auto-shapes tinted with the synthetic SASdocX
...
NOTE - what is APPROXIMATED / SKIPPED (python-pptx limits, by design):
  ...
  * New custom slide masters / layouts cannot be created from scratch by
    ...
    the default template; a SASdocX palette is applied at the shape level.
...
deterministically by the shared ``_saslib`` helper, so re-running the builder
yields a byte-identical file (CI-friendly).

Run:
    PYTHONPATH=scripts .venv/bin/python examples/builders/build_sasdocx_pptx.py
"""
...
OUT = Path(__file__).resolve().parents[1] / "templates" / "sasdocx_template.pptx"
...
# ---------------------------------------------------------------------------
# Full 12-slot SASdocX clrScheme map for the in-place theme1.xml rewrite
# (PPTX-A1), consumed VERBATIM from the shared ``_saslib`` source of truth:
...
# SASdocX theme typography pairing for the fontScheme rewrite (PPTX-A2):
...
def _set_run_theme_color(run, theme_color) -> None:
    ...
    SASdocX ``clrScheme`` authored in :func:`_rebrand_theme` rather than from a
...
def _add_stat_card(
    ...
    *,
    ...
    SASdocX theme; ``color`` still drives the text-color choice (it should be
...
def _add_cover(prs, png_path: Path):
    ...
    _set_text(
        ph[0],
        "SASdocX Corp Quarterly Business Review",
        size=40,
        bold=True,
        color=BRAND_NAVY,
    )
    # Point the cover title run at the DARK_1 theme slot so it inherits from the
    # SASdocX theme authored in _rebrand_theme (PPTX-A3); the visual color is
    ...
    if 11 in ph:  # FOOTER placeholder
        _set_text(
            ph[11],
            "SASdocX Corp - Confidential (synthetic sample)",
            size=12,
            color=BRAND_SLATE,
        )
    ...
    # Logo picture in the corner: the shared generated SASdocX wordmark.
...
def _add_content_text(prs):
    ...
    _bullets(
        ...
        [
            (
                "SASdocX Corp grew net revenue 18% YoY across all three reported regions.",
                0,
            ),
...
def _add_kpi_dashboard(prs):
    ...
    # Brand the chart-level font to the SASdocX major font (PPTX-A10): legend +
...
def _add_content_table(prs):
    ...
    with the SASdocX palette, and a totals row - exercises the table extractor
    ...
    band.text = "SASdocX Corp - FY2026 (USD thousands)"
    band.fill.solid()
...
def _add_chart(prs):
    ...
    chart = gframe.chart
    chart.has_title = True
    chart.chart_title.text_frame.text = "SASdocX Corp - FY2026 Revenue"
...
def _add_picture_slide(prs, png_path: Path):
    ...
    _set_text(ph[0], "The SASdocX Wordmark", size=28, bold=True, color=BRAND_NAVY)
    ...
    if 2 in ph:
        ...
        _bullets(
            ...
            [
                ...
                ("The logo is simply the SASdocX name, with no icon or badge.", 0),
                ("Brand guidelines:", 0),
            ],
            size=16,
        )
...
def _add_smartart_approx(prs):
    ...
    SASdocX palette. Documented as an approximation in the module docstring.
    """
    layout = prs.slide_layouts[LO_TITLE_ONLY]
    slide = prs.slides.add_slide(layout)
    ph = {p.placeholder_format.idx: p for p in slide.placeholders}
...
def _add_closing(prs):
    ...
    _bullets(
        ...
        [
            ("Questions? ", 0),
            ("SASdocX Corp - synthetic sample deck.", 0),
        ],
        size=18,
    )
    ...
    # reads "Questions? hello@sasdocx.example" with the address clickable.
    contact_para = ph[1].text_frame.paragraphs[0]
    mail_run = contact_para.add_run()
    mail_run.text = "hello@sasdocx.example"
    mail_run.font.size = Pt(18)
    mail_run.hyperlink.address = "mailto:hello@sasdocx.example"
    _set_notes(
        slide,
...
def _add_comparison(prs):
    ...
    if 4 in ph:
        _bullets(
            ph[4],
            [
                ("One SASdocX theme drives every surface.", 0),
...
def _rebrand_theme(prs) -> None:
    ...
    theme part so the deck-wide palette becomes the SASdocX palette at the
    ...
    an explicit srgbClr makes the SASdocX hex unambiguous in the surfaced
    ``theme.colors``.
    """
...
def build(out: Path = OUT) -> Path:
    out = Path(out)
    # The shared text-only SASdocX wordmark. Bytes are computed
    # deterministically by _saslib, so every example template embeds the SAME
    # logo and re-runs stay byte-identical.
    png_bytes = sasdocx_mark_png(640, 160)
    ...
    tmp_png = Path(tempfile.gettempdir()) / "sasdocx_template_wordmark.png"
    ...
    cp.author = "SASdocX Corp (synthetic fixture)"
    cp.title = "SASdocX Corp Quarterly Business Review"
    cp.subject = "Synthetic test fixture - no proprietary content"
    cp.created = fixed
    cp.modified = fixed
    cp.last_modified_by = "SASdocX Corp (synthetic fixture)"
    cp.revision = 1
