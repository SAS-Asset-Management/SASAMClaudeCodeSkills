...
Produces ``examples/templates/sasdocx_template.xlsx``: a 100% synthetic
(SASdocX-style, never proprietary) workbook that stresses the sasxlsx engine
across as many Excel component types as openpyxl can author:
  ...
  * A native TABLE object (``SASdocXDataTbl``) with a banded table style.
  ...
  * The shared text-only SASdocX wordmark placed as a worksheet drawing on the
    cover - rendered in-process by ``_saslib.sasdocx_mark_png`` (no
    external/proprietary asset on disk), sized at 4:1 so the text is not
    distorted.
  * Named cell STYLES (``SASdocXTitle``, ``SASdocXHeader``, ``SASdocXCurrency``,
    ``SASdocXPercent``, ``SASdocXInput``) registered on the workbook and applied.
...
Run:
    PYTHONPATH=scripts .venv/bin/python examples/builders/build_sasdocx_xlsx.py
"""
...
from _saslib import sasdocx_mark_png, brand_theme_slots, freeze_ooxml
from openpyxl.workbook.defined_name import DefinedName

OUT = Path(__file__).resolve().parents[1] / "templates" / "sasdocx_template.xlsx"

# Synthetic SASdocX brand palette (made-up; never proprietary).
BRAND_NAVY = "FF16213F"
BRAND_TEAL = "FF2B7CD3"
...
def _register_named_styles(wb: Workbook) -> None:
    ...
    currency = NamedStyle(name="SASdocXCurrency")
...
def _brand_band(ws, title: str, *, last_col: int = 7, band_row: int = 1) -> None:
    ...
    title_cell.style = "SASdocXBandHeader"
...
def _build_cover(wb: Workbook) -> None:
    ...
    ws["A2"].style = "SASdocXSubtitle"
    ws["A2"].alignment = Alignment(horizontal="center", vertical="center")
    ws["A4"] = "Prepared for"
    ws["B4"] = "SASdocX Corp (synthetic demo)"
    ws["A5"] = "Reporting period"
    ws["B5"] = "FY2025 (Q1-Q4)"
    # An ISO-date cell carrying the reusable SASdocXDate brand style (navy
    # Calibri + yyyy-mm-dd mask), pinned and deterministic.
    ws["A6"] = "Generated on"
    ws["B6"] = "2026-01-15"
    ws["B6"].style = "SASdocXDate"
    ...
    for anchor, label, value in scorecards:
        ...
        ws.cell(row=4, column=col, value=label).style = "SASdocXHeader"
        vc = ws.cell(row=5, column=col, value=value)
        vc.style = "SASdocXKPI"
        if value.startswith("="):
            vc.number_format = "0.0%" if "B8" in value else "#,##0"
    ws["G4"] = "Scope"
    ws["G4"].style = "SASdocXHeader"
    ws["G5"] = "3 formats"
    ws["G5"].style = "SASdocXKPI"
    ...
    # Header drawing: the shared SASdocX wordmark sits inside the printable
    ...
    ws["D11"] = "Legend"
    ws["D11"].style = "SASdocXBandHeader"
    ws.merge_cells("D11:G11")
    ws["D12"] = "Inputs"
    ws["D12"].style = "SASdocXInput"
    ...
    ws["D5"].comment = Comment(
        "Synthetic figure - pulled live from the Summary tab.", "SASdocX"
    )
...
def _build_inputs(wb: Workbook) -> None:
    ...
    for i, (driver, val, unit) in enumerate(inputs):
        r = 4 + i
        ws.cell(row=r, column=1, value=driver).style = "SASdocXInput"
        vc = ws.cell(row=r, column=2, value=val)
        vc.style = "SASdocXInput"
        if unit == "USD":
            vc.style = "SASdocXCurrency"
        elif unit == "pct":
            vc.style = "SASdocXPercent"
        elif unit == "ea":
            # Whole-unit counts get a thousands separator so "Units sold" reads
            # "29,070" instead of "29070" (keeps the input fill/border style).
            vc.number_format = "#,##0"
        ws.cell(row=r, column=3, value=unit).style = "SASdocXInput"
    ws.freeze_panes = "A4"
    ws.column_dimensions["A"].width = 18
    ...
    for r in range(4, 8):
        ws.cell(row=r, column=2).comment = Comment(
            "Synthetic input - replace with your data.", "SASdocX"
        )
...
def _build_model(wb: Workbook) -> None:
    ...
    for col, label in enumerate(headers, start=1):
        ws.cell(row=3, column=col, value=label).style = "SASdocXHeader"
    ...
    ws.cell(row=8, column=1, value="Total").style = "SASdocXTotal"
    ws.cell(row=8, column=6, value="=SUBTOTAL(109,F4:F7)").style = "SASdocXTotal"
    # A grand-total SUBTOTAL row 10 (col 6) carrying the brand total style.
    ws.cell(row=10, column=1, value="Subtotal (visible)").style = "SASdocXTotal"
    sub = ws.cell(row=10, column=6, value="=SUBTOTAL(9,F4:F6)")
    sub.style = "SASdocXTotal"
    sub.number_format = "#,##0"
    # A STRUCTURED-REFERENCE formula that reads the table's FY Total column.
    ws.cell(
        row=10, column=7, value="=SUM(SASdocXDataTbl[FY Total])"
    ).number_format = "#,##0"
    # Native TABLE object over the body (header row 3 .. data row 7 .. totals row 8).
    table = Table(displayName="SASdocXDataTbl", ref="A3:G8")
...
def _build_summary(wb: Workbook) -> None:
    ...
    ws["A3"] = "Metric"
    ws["B3"] = "Value"
    ws["A3"].style = "SASdocXHeader"
    ws["B3"].style = "SASdocXHeader"
    ...
    ws["B6"] = "=Inputs!B5"
    ws["B6"].style = "SASdocXCurrency"
    ws["A7"] = "Implied gross"
    ws["B7"] = "=Inputs!B4*Inputs!B5"
    ws["B7"].style = "SASdocXCurrency"
    ws["A8"] = "Net margin"
    ws["B8"] = "=IF(B7=0,0,B4/B7)"
    ws["B8"].style = "SASdocXPercent"
    ws["A9"] = "Headline KPI"
    ws["B9"] = "85% net margin on $1.45M gross"  # single-cell named output slot
    ws["B9"].comment = Comment(
        "Synthetic headline - derived from the model and inputs tabs.", "SASdocX"
    )
    ws.freeze_panes = "A4"
    ...
    # Repeat a small SASdocX wordmark on the summary so the printed deliverable
...
def _build_dashboard(wb: Workbook) -> None:
    ...
    ws["A14"] = "KPI trends"
    ws["A14"].style = "SASdocXBandHeader"
    trend_headers = ("KPI", "Q1", "Q2", "Q3", "Q4", "Trend")
    for col, label in enumerate(trend_headers, start=1):
        ws.cell(row=15, column=col, value=label).style = "SASdocXHeader"
    trend_rows = [
...
def _build_scenarios(wb: Workbook) -> None:
    ...
    ws["B3"].comment = Comment(
        "Synthetic scenario selector used by formulas and visual QA.", "SASdocX"
    )
    headers = ("Scenario", "Revenue multiplier", "Margin delta", "Narrative")
    for col, label in enumerate(headers, start=1):
        ws.cell(row=5, column=col, value=label).style = "SASdocXHeader"
    rows = [
    ...
    for r, row in enumerate(rows, start=6):
        for c, val in enumerate(row, start=1):
            ...
            if c == 2:
                # The revenue multiplier carries the reusable SASdocXMultiple
                # brand style (navy Calibri + '0.00x' valuation-multiple mask).
                cell.style = "SASdocXMultiple"
            elif c == 3:
                cell.style = "SASdocXPercent"
    ...
    table = Table(displayName="SASdocXScenarioTbl", ref="A5:D8")
    # The Scenarios table wears the CUSTOM SASdocXTableStyle (branded dxf header /
...
def _build_data(wb: Workbook) -> None:
    ...
    for col, label in enumerate(headers, start=1):
        ws.cell(row=1, column=col, value=label).style = "SASdocXHeader"
    demo = [
    ...
    for i, (d, region, prod, amt) in enumerate(demo):
        r = 2 + i
        # The date column carries the reusable SASdocXDate brand style.
        ws.cell(row=r, column=1, value=d).style = "SASdocXDate"
    ...
    ws.cell(row=5, column=3, value="Total").style = "SASdocXTotal"
    tot = ws.cell(row=5, column=4, value="=SUM(D2:D4)")
    tot.style = "SASdocXTotal"
    tot.number_format = "#,##0"
    ws.freeze_panes = "A2"
...
def _logo_path() -> Path:
    """Materialize the shared SASdocX wordmark to a temp file openpyxl can
    embed.

    Uses ``sasdocx_mark_png`` so the cover logo is the SAME generated
    ...
    if _LOGO_CACHE is None:
        ...
        tmp = Path(tempfile.gettempdir()) / "sasdocx_template_wordmark.png"
        tmp.write_bytes(sasdocx_mark_png(640, 160))
        _LOGO_CACHE = tmp
    return _LOGO_CACHE
...
#   * theme1.xml a:clrScheme + a:fontScheme  -> the workbook-level SASdocX palette
#   * styles.xml  custom dxf-backed table style 'SASdocXTableStyle'
...
# _saslib slot map so the .xlsx theme can never drift from the .docx/.pptx one.
...
# Full SASdocX clrScheme, consumed VERBATIM from the shared source of truth:
...
def _rewrite_theme(xml: bytes) -> bytes:
    """Rewrite theme1.xml's a:clrScheme to the SASdocX palette + Arial/Calibri.
...
# plus a <tableStyle name="SASdocXTableStyle"> referencing them.
def _inject_table_style(xml: bytes) -> bytes:
    """Add the SASdocXTableStyle (header/stripe/total dxf bands) to styles.xml."""
    root = etree.fromstring(xml)
    ...
    style.set("name", "SASdocXTableStyle")
    style.set("pivot", "0")
    style.set("count", "3")
    for etype, dxf_id in (
        ("headerRow", header_id),
