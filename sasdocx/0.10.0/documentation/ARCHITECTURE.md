...
Most "use my template" tools copy *styling*. SASdocX also learns the template's
...
SASdocX is designed so reliability is visible, testable, and improvable. The
...
Point SASdocX at one branded `.docx`, `.pptx` or `.xlsx`. It `extract`s a
...
lost. SASdocX is faithful **by construction**: generators never write a literal
...
Yes. The three skills (`sasdocx`, `saspptx`, `sasxlsx`) are plain agent
