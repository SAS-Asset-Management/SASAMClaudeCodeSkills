...
|---|---|---|
| **`sasdocx`** | Word `.docx` | reports, letters, memos: cover, headings, paragraphs, callouts, quotes, captions, lists, tables, in the template's structural order |
| **`saspptx`** | PowerPoint `.pptx` | decks: title / section / content slides from the template's real masters & layouts, with real bullet levels and long-text splitting |
| **`sasxlsx`** | Excel `.xlsx` | workbooks: fills named cells & regions while **preserving formulas** and workbook structure |
...
**Alpha, maturing.** The Word vertical (`sasdocx`) is the reference
...
| `sasdocx`: extract → verify → generate | ✅ working |
...
| `saspptx`: roles from real layouts, native charts / SmartArt / merged tables | ✅ working (fidelity still catching up to docx) |
| `sasxlsx`: named-region fills, formula-preserving, native charts | ✅ working (fidelity still catching up to docx) |
