...
SASdocX is **local-first**: it runs entirely on your machine, with no cloud
...
> (`sasdocx`, `saspptx`, `sasxlsx`) share a single Python engine in
> [`scripts/sasdockit/`](../scripts/sasdockit). Each skill is a thin `SKILL.md` plus
...
```bash
git clone https://github.com/ferdinandobons/sasdocx.git
cd sasdocx
...
```bash
python scripts/sasdockit/cli.py doctor
```
...
```bash
python scripts/sasdockit/cli.py doctor --json   # machine-readable probe dict; skips the human report
python scripts/sasdockit/cli.py doctor --fast   # skip the slow LibreOffice render smoke-test; marks visual QA "not probed"
```
...
```text
/plugin marketplace add ferdinandobons/sasdocx
/plugin install sasdocx@sasdocx
```
...
```bash
git submodule add https://github.com/ferdinandobons/sasdocx.git .agents/sasdocx
cd .agents/sasdocx && python3 -m venv .venv && . .venv/bin/activate && pip install -r requirements.txt
```

### Codex / other agents (clone + symlink)

```bash
git clone https://github.com/ferdinandobons/sasdocx.git ~/.codex/sasdocx
cd ~/.codex/sasdocx && python3 -m venv .venv && . .venv/bin/activate && pip install -r requirements.txt
mkdir -p ~/.codex/skills
for s in sasdocx saspptx sasxlsx; do ln -s ~/.codex/sasdocx/skills/$s ~/.codex/skills/$s; done
```

The symlink keeps each skill's engine reachable, since `scripts/sasdockit/` stays
in the cloned repo.
...
```text
/plugin update sasdocx@sasdocx
```

If the update doesn't pick up new changes, refresh the marketplace source and reinstall:

```text
/plugin uninstall sasdocx@sasdocx
/plugin marketplace remove sasdocx
/plugin marketplace add ferdinandobons/sasdocx
/plugin install sasdocx@sasdocx
```
...
```bash
git submodule update --remote .agents/sasdocx
```
