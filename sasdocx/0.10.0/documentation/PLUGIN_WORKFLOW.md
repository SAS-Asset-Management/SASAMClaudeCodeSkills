# SASdocX Plugin Workflow

This document is the operational map for the SASdocX plugin and its three
format skills (`sasdocx`, `saspptx`, `sasxlsx`). It describes how an
...
|---|---|---|
| `sasdocx` | Word `.docx` | branded reports, memos, letters |
| `saspptx` | PowerPoint `.pptx` | branded decks |
| `sasxlsx` | Excel `.xlsx` | branded workbooks |

The slash commands in `commands/` are thin wrappers over the internal CLI:

| Command | CLI verb |
|---|---|
| `/sasdocx-extract` | `python scripts/sasdockit/cli.py extract ...` |
| `/sasdocx-verify` | `python scripts/sasdockit/cli.py verify ...` |
| `/sasdocx-generate` | `python scripts/sasdockit/cli.py generate ...` |
| `/sasdocx-list` | `python scripts/sasdockit/cli.py list ...` |
...
```mermaid
flowchart TD
    U["User request + Office template/content"] --> P["sasdocx plugin"]
