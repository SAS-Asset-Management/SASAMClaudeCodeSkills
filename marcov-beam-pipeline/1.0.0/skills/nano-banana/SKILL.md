...
**Use Cases:**
- Presentation visuals for MBP:presentation
- Social media graphics for MBP:linkedin-post
- Industry/market infographics for MBP:b2b-research
- Proposal visuals for MBP:beam-selling
...
```bash
node ~/.claude/SASAMClaudeCodeSkills/marcov-beam-pipeline/1.0.0/skills/nano-banana/scripts/post-process.js <input.png> <output_final.jpg>
```
...
```
/MBP:nano-banana setup                                    # First-time configuration
/MBP:nano-banana status                                   # Show config and stats
/MBP:nano-banana "A futuristic city skyline at sunset"   # Generate image
/MBP:nano-banana "Professional infographic about AI trends" --aspect 16:9 --size 2K
/MBP:nano-banana history                                  # Recent generations
```
...
Before generating any image, check if `~/.claude/skills/MBP-nano-banana/config.json` exists.

**If config exists:**
1. Load configuration from `~/.claude/skills/MBP-nano-banana/config.json`
...
Note: The API key will be stored globally in ~/.claude/skills/MBP-nano-banana/config.json
...
Store configuration in `~/.claude/skills/MBP-nano-banana/config.json`:
...
**Security:**
- Config is stored in `~/.claude/skills/MBP-nano-banana/` (outside project directory)
...
node ~/.claude/SASAMClaudeCodeSkills/marcov-beam-pipeline/1.0.0/skills/nano-banana/scripts/post-process.js "./generated-images/${FILENAME}.png" "./generated-images/${FILENAME}_final.jpg"
...
1. **Run post-processing script** (REQUIRED):
   ```bash
   node ~/.claude/SASAMClaudeCodeSkills/marcov-beam-pipeline/1.0.0/skills/nano-banana/scripts/post-process.js <input.png> <output.jpg>
   ```
...
```bash
node ~/.claude/SASAMClaudeCodeSkills/marcov-beam-pipeline/1.0.0/skills/nano-banana/scripts/post-process.js ./generated-images/IMAGE.png ./generated-images/IMAGE_final.jpg
```
...
Assets located at:
```
~/.claude/SASAMClaudeCodeSkills/marcov-beam-pipeline/1.0.0/skills/nano-banana/assets/
...
|--------------|-----------------|---------------------|
| MBP:presentation | "Generate visual for slide about [topic]" | 16:9, 2K |
| MBP:linkedin-post | "Create image for LinkedIn post" | 1:1 or 4:5, 2K |
| MBP:b2b-research | "Generate infographic for [company/market]" | 16:9 or 1:1, 2K |
| MBP:beam-selling | "Create visual for proposal section" | 16:9, 2K |

### Cross-Skill Example

When MBP:presentation needs a visual:

1. MBP:presentation identifies need for visual asset
2. Check if MBP:nano-banana is configured
3. Generate appropriate prompt based on slide content
4. Call MBP:nano-banana with 16:9 aspect ratio
...
Run `/MBP:nano-banana setup` to configure your Google AI Studio API key.
...
2. Run `/MBP:nano-banana setup` to enter a new key
```
...
When the user runs `/MBP:nano-banana status`:
...
- [ ] Configuration loaded from `~/.claude/skills/MBP-nano-banana/config.json`
